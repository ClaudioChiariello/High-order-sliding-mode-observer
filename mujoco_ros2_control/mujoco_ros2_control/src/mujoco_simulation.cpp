                                  ...
                                  const std::string& mujoco_model_topic, double sim_speed_factor, bool headless)
{
  ...
  headless_ = headless;
  getSimData_ = std::make_unique<GetSimData>(this, node_);
...
void GetSimData::print_all_joint_torques(const mjModel* m, const mjData* d) {
...
void GetSimData::publish_wrenches(){
    std::lock_guard<std::recursive_mutex> lock(sim_->getMutex());

    
    int body_id = mj_name2id(sim_->getModel(), mjOBJ_BODY, "base_link");

    publish_odometry(body_id);

    getGroundContactWrench(body_id);
    publish_imu_data_kinematics(body_id);

    // double lin[3] = {0.0, 0.0, 0.0};
    // double ang[3] = {0.0, 0.0, 0.0};
    // if (getBodyVelocity("base_link", lin, ang, true))
    // { 
      
    //   if(count_ % 500 == 0){
    //     std::cout << "Linear:  " << lin[0] << " " << lin[1] << " " << lin[2] << std::endl;
    //     std::cout << "Angular: " << ang[0] << " " << ang[1] << " " << ang[2] << std::endl;
    //   }
    //   count_++;
    // }

    // print_all_joint_torques(sim_->getModel(), sim_->getData());
    //check_rear_wheel_velocities(sim_->getModel(), sim_->getData());

    const mjtNum* wrench = sim_->getData()->cfrc_int + 6 * body_id;
  ...
  void GetSimData::publish_imu_data_kinematics(int body_id){

    std::lock_guard<std::recursive_mutex> lock(sim_->getMutex());

    const mjModel* model = sim_->getModel();
    mjData* data = sim_->getData();
    const char* body_name_char = mj_id2name(sim_->getModel(), mjOBJ_BODY, body_id);
    if (body_id < 0) return;
...
void GetSimData::getGroundContactWrench(int body_id){
  ...
  void GetSimData::publish_odometry(int body_id)
  {
    ...
    double pos_x = sim_->getData()->xpos[3 * body_id + 0];
    double pos_y = sim_->getData()->xpos[3 * body_id + 1];
    double pos_z = sim_->getData()->xpos[3 * body_id + 2];

    // 2. Retrieve orientation quaternion (w, x, y, z) from mjData
    double quat_w = sim_->getData()->xquat[4 * body_id + 0];
    double quat_x = sim_->getData()->xquat[4 * body_id + 1];
    double quat_y = sim_->getData()->xquat[4 * body_id + 2];
    double quat_z = sim_->getData()->xquat[4 * body_id + 3];

    // retrieve the velocity in the world frame
    mjtNum vel[6];
    mj_objectVelocity(sim_->getModel(), sim_->getData(), mjOBJ_BODY, body_id, vel, 0);
    ...
    mjtNum* R = &sim_->getData()->xmat[body_id * 9];
